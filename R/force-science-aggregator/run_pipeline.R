#!/usr/bin/env Rscript
# Main Pipeline for Force Science News Aggregator
...
# Source all modules
source(here("R", "force-science-aggregator", "utils.R"))
source(here("R", "force-science-aggregator", "scrape_rss.R"))
source(here("R", "force-science-aggregator", "scrape_newsapi.R"))
source(here("R", "force-science-aggregator", "process_stories.R"))
source(here("R", "force-science-aggregator", "generate_output.R"))
...
main <- function(config_path = NULL) {
  ...
  log_message("=== Force Science News Aggregator Pipeline Started ===", log_file)
  log_message(sprintf("Run time: %s", Sys.time()), log_file)

  # Load configuration
  if (is.null(config_path)) {
    config_path <- here("config", "force-science-sources.yaml")
  }
  ...
  # Initialize database
  db_path <- here("data", "force-science", "stories.sqlite")
  ...
  tryCatch(
    {
      ...
      if (config$newsapi$enabled %||% FALSE) {
        newsapi_articles <- search_newsapi_multiple(
          ...
          from_date = format(Sys.Date() - (config$newsapi$days_back %||% 7), "%Y-%m-%d"),
          log_file = log_file
        )
      ...
      if (nrow(all_articles) == 0) {
        ...
        if (nrow(new_articles) > 0) {
          # Step 6: Process and filter articles
          log_message("--- Step 6: Processing Articles ---", log_file)
          ...
          if (nrow(processed_articles) > 0) {
            ...
            log_message("No articles passed relevance filtering", log_file, "WARN")
          }
        }
      }
