#!/usr/bin/env Rscript
# Main Pipeline for AI Police News Aggregator
...
# Source all modules
source(here("R", "news-aggregator", "utils.R"))
source(here("R", "news-aggregator", "scrape_rss.R"))
source(here("R", "news-aggregator", "scrape_newsapi.R"))
source(here("R", "news-aggregator", "process_stories.R"))
source(here("R", "news-aggregator", "generate_output.R"))
...
main <- function(config_path = NULL) {
  ...
  log_message("=== AI Police News Aggregator Pipeline Started ===", log_file)
  ...
  # Initialize database
  db_path <- here("data", "ai-police-news", "stories.sqlite")
