# Output Generation for Police K9 Incidents
...
source(here::here("R", "k9-aggregator", "utils.R"))
...
generate_json_output <- function(con, output_path = NULL, max_stories = 100, log_file = NULL) {
  if (is.null(output_path)) {
    output_path <- here::here("static", "data", "k9-incidents.json")
  }
...
generate_rss_feed <- function(con, output_path = NULL, max_stories = 50, log_file = NULL) {
  if (is.null(output_path)) {
    output_path <- here::here("static", "data", "k9-incidents.xml")
  }
  ...
  xml_add_child(channel, "title", "Research Watch: Police K9 Incidents")
  xml_add_child(channel, "link", "https://ianadamsresearch.com/k9-incidents/")
  xml_add_child(channel, "description", "Curated news and research on police K9 bites, incidents, legal cases, and policy")
...
generate_markdown_stories <- function(con, output_dir = NULL, log_file = NULL) {
  if (is.null(output_dir)) {
    output_dir <- here::here("data", "k9-incidents", "stories")
  }
...
generate_review_report <- function(con, output_path = NULL, log_file = NULL) {
  if (is.null(output_path)) {
    output_path <- here::here("data", "k9-incidents", "needs_review.json")
  }
