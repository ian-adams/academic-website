# Output Generation for Force Science News
...
generate_json_output <- function(con, output_path = NULL, max_stories = 100, log_file = NULL) {
  if (is.null(output_path)) {
    output_path <- here::here("static", "data", "force-science-news.json")
  }
  ...
  if (nrow(stories) == 0) {
    ...
    stories_list <- stories %>%
      ...
      select(
        ...
        key_entities, location, tags, needs_review
      ) %>%
      pmap(function(...) {
        story <- list(...)
...
generate_rss_feed <- function(con, output_path = NULL, max_stories = 50, log_file = NULL) {
  if (is.null(output_path)) {
    output_path <- here::here("static", "data", "force-science-news.xml")
  }
  ...
  xml_add_child(channel, "title", "Research Watch: Force Science Institute News")
  xml_add_child(channel, "link", "https://ianadamsresearch.com/force-science-news/")
  xml_add_child(channel, "description", "Curated news tracking the Force Science Institute's influence on police use-of-force testimony, legal proceedings, and policy")
  ...
  if (nrow(stories) > 0) {
    for (i in 1:nrow(stories)) {
      ...
      description <- sprintf(
        "%s<br/><br/><strong>Source:</strong> %s<br/><strong>Type:</strong> %s<br/><strong>Relevance:</strong> %.0f%% (%s confidence)",
        story$summary,
        story$source,
        tools::toTitleCase(story$story_type),
        story$relevance_score * 100,
        story$relevance_confidence %||% "unknown"
      )
      ...
      if (!is.na(story$key_entities) && story$key_entities != "") {
        description <- paste0(description, sprintf("<br/><strong>Entities:</strong> %s", story$key_entities))
      }
...
generate_markdown_stories <- function(con, output_dir = NULL, log_file = NULL) {
  if (is.null(output_dir)) {
    output_dir <- here::here("data", "force-science", "stories")
  }
  ...
  for (i in 1:nrow(stories)) {
    ...
    frontmatter <- c(
      ...
      sprintf("confidence: \"%s\"", story$relevance_confidence %||% "unknown")
    )
...
generate_review_report <- function(con, output_path = NULL, log_file = NULL) {
  if (is.null(output_path)) {
    output_path <- here::here("data", "force-science", "needs_review.json")
  }
  ...
  if (nrow(stories) == 0) {
    ...
    output <- list(
      ...
      count = nrow(stories),
      stories = stories %>%
        select(id, title, url, source, date_published, relevance_score, relevance_confidence, story_type, summary) %>%
        pmap(list)
    )
