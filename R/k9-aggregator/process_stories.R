# Story Processing and Filtering for K9 Incidents
...
apply_negative_filters <- function(articles, log_file = NULL) {
  ...
  # Negative keywords that indicate completely unrelated stories
  negative_patterns <- c(
    "facial recognition",
    "predictive policing",
  ...
  # Only filter if the story is ONLY about these topics and doesn't mention K9/canine
  articles_filtered <- articles %>%
    filter(
      !(
        str_detect(tolower(paste(title, summary)), paste(negative_patterns, collapse = "|")) &
          !str_detect(tolower(paste(title, summary)), "k9|canine|dog")
      )
    )
