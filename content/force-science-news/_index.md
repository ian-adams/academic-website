...
<div id="force-science-feed" class="force-science-feed">
...
(function() {
  ...
  function renderFeed() {
    const container = document.getElementById('force-science-feed');
    ...
    const types = ['legal', 'daubert', 'academic', 'policy', 'investigative', 'prosecutorial', 'international', 'opinion'];
    ...
    filteredStories.forEach(function(story) {
      ...
      const confidence = story.relevance_confidence || 'unknown';

      // Handle tags as either array or string
      let tags = '';
      if (story.tags) {
        const tagArray = Array.isArray(story.tags) ? story.tags : [story.tags];
        tags = tagArray.slice(0, 8).map(function(tag) { return '<span class="tag">' + tag + '</span>'; }).join('');
      }

      // Handle key_entities as either array or string
      let entities = '';
      if (story.key_entities) {
        const entityArray = Array.isArray(story.key_entities) ? story.key_entities : story.key_entities.split(', ');
        entities = entityArray.join(', ');
      }
      ...
      storiesHTML += '<div class="news-story" data-type="' + (story.story_type || 'general') + '">' +
        ...
        '<span class="meta-item"><span class="confidence-badge confidence-' + confidence + '">' + confidence + '</span></span>' +
        '<span class="meta-item">' + formatDate(story.date || story.date_discovered) + '</span>' +
        '<span class="meta-item">' + story.source + '</span>' +
        (story.location ? '<span class="meta-item">' + story.location + '</span>' : '') +
        '<span class="meta-item"><span class="relevance-score relevance-' + relevanceClass + '">' + relevancePercent + '%</span></span>' +
        '</div>' +
        '<div class="story-summary">' + story.summary + '</div>' +
        (entities ? '<div class="entities-list"><strong>Entities:</strong> ' + entities + '</div>' : '') +
