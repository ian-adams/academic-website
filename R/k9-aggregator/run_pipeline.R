#!/usr/bin/env Rscript
# Main Pipeline for Police K9 Incident Aggregator
...
# Source all modules
source(here("R", "k9-aggregator", "utils.R"))
source(here("R", "k9-aggregator", "scrape_rss.R"))
source(here("R", "k9-aggregator", "scrape_newsapi.R"))
source(here("R", "k9-aggregator", "process_stories.R"))
source(here("R", "k9-aggregator", "generate_output.R"))
...
main <- function(config_path = NULL) {
  ...
  log_message("=== Police K9 Incident Aggregator Pipeline Started ===", log_file)
  log_message(sprintf("Run time: %s", Sys.time()), log_file)

  # Load configuration
  if (is.null(config_path)) {
    config_path <- here("config", "k9-incident-sources.yaml")
  }
  ...
  # Initialize database
  db_path <- here("data", "k9-incidents", "stories.sqlite")
